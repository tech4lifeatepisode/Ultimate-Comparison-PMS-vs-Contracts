...
const OUTPUT_DIR = path.join(__dirname, 'Blind Contracts');
...
async function blindContract(fileName, fileBuffer) {
  const pages = await extractPages(fileBuffer);
  const boxes = findAllRedactionBoxes(pages);

  if (boxes.length === 0) {
    throw new Error('No redaction regions found (De otra parte / El Cliente signature).');
  }

  const page1Count = boxes.filter((b) => b.page === 1).length;
  const sigCount = boxes.length - page1Count;
  console.log(`  Page 1 (Cliente/Avalista parties): ${page1Count} region(s)`);
  console.log(`  Signature (El Cliente / El Avalista): ${sigCount} region(s)`);
  console.log(`  Rasterizing ${new Set(boxes.map((b) => b.page)).size} page(s) — text not copyable on those pages`);

  return applyRedactions(fileBuffer, boxes);
}
...
async function main() {
  ...
  if (files.length === 0) {
    console.log(`No PDF files found in "${path.basename(CONTRACTS_DIR)}". Add contracts and run again.`);
    process.exit(0);
  }

  console.log(`Blinding ${files.length} contract(s) → "${path.basename(OUTPUT_DIR)}/"\n`);

  let failed = 0;
  for (const name of files) {
    console.log(`Processing: ${name}`);
    const inputPath = path.join(CONTRACTS_DIR, name);
    const outputPath = path.join(OUTPUT_DIR, name);

    try {
      const buffer = await fs.readFile(inputPath);
      const blinded = await blindContract(name, buffer);
